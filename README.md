...
Для полной пересборки проекта с нуля (удаление всех контейнеров, томов и данных) используйте скрипт `scripts/rebuild.sh`:

```bash
# Интерактивный режим с подтверждениями
./scripts/rebuild.sh

# Автоматический режим без подтверждений (осторожно!)
./scripts/rebuild.sh --force
```

Скрипт выполняет следующие действия:
1. Останавливает и удаляет все контейнеры проекта
2. Опционально удаляет все тома Docker, связанные с проектом
3. Опционально удаляет все локальные данные (mongo_data, redis_data, minio_data)
4. Опционально очищает директории frontend и backend, сохраняя только файлы, необходимые для сборки (Dockerfile и entrypoint.sh)
5. Опционально удаляет сертификаты для генерации новых
6. Опционально удаляет образы Docker, связанные с проектом
7. Запускает скрипт развертывания для пересоздания проекта

При сборке Docker-образов код автоматически загружается из GitHub-репозиториев:
- Backend: https://github.com/viktormokhov/reading-and-get-stars-tgbot-backend.git
- Frontend: https://github.com/viktormokhov/reading-and-get-stars-tgbot-frontend.git

**ВНИМАНИЕ**: Этот скрипт удаляет все данные! Перед использованием рекомендуется создать резервную копию с помощью `scripts/backup.sh`.
